...
########################################################################################################################
class MenuItem():
    def __init__(self, label:nextcord.Emoji, func, custom_id, short_description, help_text) -> None:
        self.label = label
        self.func = func
...
class AsyncRaceButton(nextcord.ui.Button):
    def __init__(self, menu_item: MenuItem, payload = None, *, style: ButtonStyle = ButtonStyle.primary, disabled: bool = False, url: str | None = None, emoji: str | Emoji | PartialEmoji | None = None, row: int | None = None) -> None:
        self.menu_item = menu_item
        self.payload = payload
        super().__init__(style=style, label=None, disabled=disabled, custom_id=menu_item.custom_id, url=url, emoji=PartialEmoji.from_str(menu_item.label), row=row)
...
class RaceButtonMenu(menus.ButtonMenu):
    def __init__(self, payload, menu_item_list: list[MenuItem], *, title: str = "", description: str = "", footer: str = DefaultFooter, color: int = 0x5865F2) -> None:
...
async def category_edit_scoring(interaction, category):
    ...
    for s in select_list:
        if s.value == category.points_type:
            s.default = True
            break
    view = zSingleSelectView(select_list, None, "Choose Scoring Method...")
    await send_message(interaction, view=view)
    await view.wait()
    selected = view.get_selected_value()
...
async def category_edit_submit_role(interaction, category):
    ...
    view = zSingleSelectView(role_list, None, "Choose Role...")
    await send_message(interaction, view=view)

    # Wait for the user to select a role
    await view.wait()

    # Save the selected role to the category
    selected_role = view.get_selected_value()
    selected_role = None if selected_role == 0 else selected_role
...
async def category_edit_leaderboard_channel(interaction, category):
    ...
    view = zSingleSelectView(channel_list, None, "Choose Channel...")
    await send_message(interaction, view=view)

    # Wait for the user to select a channel
    await view.wait()

    # Save the selected channel to the category
    selected_channel = view.get_selected_value()
    selected_channel = None if selected_channel == 0 else selected_channel
    logging.info(f"{interaction.user.display_name} selected leaderboard channel: {selected_channel} for category {category.id}")
    category.leaderboard_channel = selected_channel
    category.save()
...
async def send_category_menu(interaction, category_id):
    ...
    menu = RaceButtonMenu(category, CategoryButtonMenuItems, title=title, description=description)
...
async def send_race_menu(interaction, race_id):
    ...
    menu = RaceButtonMenu(race, RaceButtonMenuItems, title=title, description=description)
    await menu.start(interaction=interaction, ephemeral=True)
